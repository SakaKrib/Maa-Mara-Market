...
const PendingOrdersPage = () => {
  const { period, setPeriod, series, value, loading, error } = useVendorAnalytics("pending_orders");
    ...
    <section className="w-full space-y-5">
      <header className="rounded-2xl border border-[#e6e6e4] bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><h1 className="mt-1 text-2xl font-bold tracking-tight text-[#222]">Pending Orders</h1><p className="mt-1 text-sm text-[#595959]">Pending orders grouped by the selected period.</p></div>
      ...
      <section className="rounded-2xl border border-[#e6e6e4] bg-white p-4 shadow-sm sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-3 border-b border-[#e6e6e4] pb-4">
          <div><h2 className="text-lg font-bold text-[#222]">Pending-order trend</h2><p className="mt-1 text-xs text-[#595959]">{PERIOD_OPTIONS.find((x) => x.value === period)?.description}</p></div>
        ...
        {loading ? <div className="mt-5 h-[320px] rounded-xl bg-[#f8f8f6] p-8 text-sm text-[#595959]">Loading...</div> : error ? <div className="mt-5 rounded-xl bg-red-50 p-8 text-sm text-red-600">{error}</div> : series.length ? (
          <div className="mt-5 h-[320px] w-full min-w-0 sm:h-[380px]"><ResponsiveContainer width="100%" height="100%"><LineChart data={series} margin={{top:8,right:8,left:0,bottom:8}}><CartesianGrid strokeDasharray="3 3" stroke="#e6e6e4" vertical={false}/><XAxis dataKey="name" tick={{fontSize:10,fill:"#595959"}} tickLine={false} axisLine={{stroke:"#d9d9d6"}}/><YAxis tick={{fontSize:10,fill:"#595959"}} tickLine={false} axisLine={false} width={38}/><Tooltip contentStyle={{borderRadius:12,border:"1px solid #e6e6e4",background:"#fff"}}/><Line type="monotone" dataKey="pv" stroke="#c2410c" strokeWidth={3} dot={{r:3}} activeDot={{r:5}}/></LineChart></ResponsiveContainer></div>
        ) : <div className="mt-5 rounded-xl bg-[#f8f8f6] p-8 text-center text-sm text-[#595959]">No pending orders for this period.</div>}
      </section>
    </section>
  );
};
