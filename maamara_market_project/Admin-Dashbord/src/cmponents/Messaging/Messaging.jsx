        ...
        <aside className={(mobileThread ? "hidden lg:flex" : "flex") + " w-full shrink-0 flex-col border-r border-[#e6e6e4] lg:w-[340px]"}>
          <div className="border-b border-[#e6e6e4] p-4 text-center">
            <div className="flex flex-col items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#fff0e8] text-[#f1641e]"><IonIcon icon={chatbubbleEllipsesOutline} className="text-xl" /></div>
            ...
            {isAdmin ? (
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setTab("users")} className={"rounded-xl px-3 py-2 text-xs font-bold " + (tab === "users" ? "bg-[#f1641e] text-[#f1641e]-foreground" : "bg-[#f8f8f6] text-[#595959]")}>Users</button>
                <button type="button" onClick={() => setTab("vendors")} className={"rounded-xl px-3 py-2 text-xs font-bold " + (tab === "vendors" ? "bg-[#f1641e] text-[#f1641e]-foreground" : "bg-[#f8f8f6] text-[#595959]")}>Vendors</button>
              </div>
            ) : (
              <button type="button" onClick={() => contacts.admins?.[0] && startConversation(contacts.admins[0])} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-[#f1641e] px-4 py-2.5 text-xs font-bold text-[#f1641e]-foreground"><IonIcon icon={addOutline} /> Start a chat with Admin</button>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {conversations.map((conversation) => (
              <button key={conversation.id} type="button" onClick={() => selectConversation(conversation)} className={"flex w-full gap-3 border-b border-[#e6e6e4] p-3 text-left hover:bg-[#f8f8f6] " + (selected?.id === conversation.id ? "bg-[#f8f8f6]" : "")}>
                <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#fff0e8] text-[#f1641e]"><IonIcon icon={isAdmin && other(conversation)?.is_vendor ? storefrontOutline : peopleOutline} /></div>
                <div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><p className="truncate text-sm font-semibold text-[#222]">{nameOf(conversation)}</p>{conversation.unread_count > 0 && <span className="rounded-full bg-[#f1641e] px-2 py-0.5 text-[10px] text-[#f1641e]-foreground">{conversation.unread_count}</span>}</div><p className="truncate text-xs text-[#595959]">{conversation.last_message?.body || (conversation.last_message?.image ? "Image" : "No messages yet")}</p></div>
              </button>
            ))}
            ...
            <div className="border-t border-[#e6e6e4] p-3">
              ...
              {activeContacts.map((contact) => (
                <button key={contact.id} type="button" onClick={() => startConversation(contact)} className="flex w-full items-center gap-3 rounded-xl p-2 text-left hover:bg-[#f8f8f6]">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#fff0e8] text-[#f1641e]"><IonIcon icon={contact.is_vendor ? storefrontOutline : personOutline} /></div>
          ...
          {selected ? (
            <>
              <header className="flex items-center gap-3 border-b border-[#e6e6e4] p-3">
                ...
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#fff0e8] text-[#f1641e]"><IonIcon icon={chatbubbleEllipsesOutline} /></div>
              ...
              <div className="flex-1 overflow-y-auto bg-[#f8f8f6] p-3 sm:p-5">
                {messages.map((message) => {
                  ...
                  return <div key={message.id} className={"mb-3 flex " + (mine ? "justify-end" : "justify-start")}><div className={"max-w-[82%] rounded-2xl px-3 py-2 " + (mine ? "rounded-br-md bg-[#f1641e] text-[#f1641e]-foreground" : "rounded-bl-md bg-[#f8f8f6] text-[#222]")}>{message.image && <img src={resolveApiAssetUrl(message.image)} alt="Attachment" className="mb-2 max-h-72 max-w-full rounded-xl object-contain" />}{message.body && <p className="whitespace-pre-wrap break-words text-sm">{message.body}</p>}<p className="mt-1 text-[9px] opacity-70">{timeLabel(message.created_at)}</p></div></div>;
                })}
                <div ref={endRef} />
              </div>
              ...
              <form onSubmit={sendMessage} className="border-t border-[#e6e6e4] p-3">
                <div className="flex items-end gap-2 rounded-2xl border border-[#e6e6e4] bg-[#f8f8f6] p-2">
                  ...
                  <button type="submit" disabled={sending || (!body.trim() && !image)} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#f1641e] text-[#f1641e]-foreground disabled:opacity-50"><IonIcon icon={sendOutline} /></button>
                </div>
              </form>
            </>
          ) : (
            <div className="flex flex-1 flex-col items-center justify-center p-8 text-center"><div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#fff0e8] text-[#f1641e]"><IonIcon icon={chatbubbleEllipsesOutline} className="text-3xl" /></div><h2 className="mt-4 text-lg font-bold text-[#222]">Start a conversation</h2><p className="mt-1 text-sm text-[#595959]">{isAdmin ? "Choose a user or vendor." : "Start a conversation with the Maa Mara admin team."}</p></div>
          )}
        </main>
      </div>
    </div>
