  ...
  <div className="flex min-w-0 items-center justify-between gap-4">
    <div className="min-w-0">
      ...
      <div className="mt-1.5 flex items-center gap-2">
        <span className={`text-xs font-bold ${percentageColor.includes("text-") ? percentageColor : "text-[#2563eb]"}`}>{percentage}</span>
        <span className="truncate text-[11px] text-[#777]">{duration}</span>
      </div>
      {link && <Link to={link} className="mt-2 inline-flex text-xs font-semibold text-[#2563eb] hover:text-[#1d4ed8]">View all</Link>}
    </div>
    <div className="h-20 w-[42%] min-w-[105px] max-w-[170px]">
      {chartData.length > 0 ? (
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            <Tooltip contentStyle={{ border: "1px solid #e6e6e4", borderRadius: 10, background: "#fff", fontSize: 11 }} labelStyle={{ display: "none" }} />
