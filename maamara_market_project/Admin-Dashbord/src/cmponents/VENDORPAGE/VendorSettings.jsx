  ...
  return (
    <section className="space-y-5">
      <header className="flex items-start justify-between gap-4 rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-5">
        ...
        <button type="button" onClick={() => navigate("/vendors-dashboard")} className="rounded-xl border border-border bg-muted px-3 py-2 text-xs font-semibold text-muted-foreground hover:bg-primary hover:text-primary-foreground">
          Back
        </button>
      </header>
      ...
      {loading ? (
        ...
        <div className="grid gap-5 lg:grid-cols-[1.5fr_1fr]">
          <form onSubmit={updateProfile} className="rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-5">
            ...
            <button type="submit" disabled={saving} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-60">
