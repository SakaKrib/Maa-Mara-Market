...
const onSubmit = async (data) => {
  try {
    ...
    if (data.manufactured_date) formData.append("manufactured_date", data.manufactured_date);
    if (data.expiry_date) formData.append("expiry_date", data.expiry_date);
    if (data.roast_type) formData.append("roast_type", data.roast_type);
    if (data.coffee_state) formData.append("coffee_state", data.coffee_state);
    if (data.item_attribute) formData.append("item_attribute", JSON.stringify(data.item_attribute));
    ...
    (data.color_variants || []).forEach((variant, i) => {
      ...
      formData.append(`variants[${i}][sizes]`, JSON.stringify(variant.sizes || []));
      if (variant.color_image instanceof File) {
        formData.append(`variants[${i}][image]`, variant.color_image);
      }
    });
    ...
    if (data.shoe_input?.length > 0) {
      formData.append("shoe_input", JSON.stringify(data.shoe_input));
    }

    // ------------------------
    // ✅ Size Variants
    // ------------------------
    formData.append("size_only_icon", JSON.stringify(data.size_variant || []));
    formData.append("kids_sizes", JSON.stringify(data.kids_sizes || []));

    // ------------------------
    // ✅ Color Variants
    // ------------------------
    const variantsPayload = (data.color_variants || []).map((variant) => ({
      color: variant.color,
      sizes: (variant.sizes || []).map((s) => ({
        size: s.size,
        quantity_in_stock: s.quantity_in_stock ?? s.stock ?? 0,
      })),
      has_image: variant.color_image instanceof File,
    }));
    formData.append("variants", JSON.stringify(variantsPayload));

    // Attach variant images separately
    (data.color_variants || []).forEach((variant, i) => {
      if (variant.color_image instanceof File) {
        formData.append(`variant_images_${i}`, variant.color_image);
      }
    });
    ...
    if (data.shipping_dimension_data) {
      formData.append("shipping_dimension[length]", data.shipping_dimension_data.length);
      formData.append("shipping_dimension[width]", data.shipping_dimension_data.width);
      formData.append("shipping_dimension[height]", data.shipping_dimension_data.height);
      if (data.shipping_dimension_data.weight) {
        formData.append("shipping_dimension[weight]", data.shipping_dimension_data.weight);
      }
    }
    ...
    if (data.in_offer && data.offer) {
      formData.append("offer_discount_percentage", (data.offer.discount_percentage ?? 0).toString());
    ...
    // ✅ Length & Weight
    // ------------------------
    if (data.length?.value) formData.append("length", JSON.stringify(data.length));
    if (data.weight?.value) formData.append("weight", JSON.stringify(data.weight));
    ...
    alert(
      "Update failed: " +
        (error.response?.data ? JSON.stringify(error.response.data) : "Network error")
    );
  }
};
