...
const ItemEditForm = ({ form, vendorId, itemId, item, vendor }) => {
  ...
  if (item.section === "organic") {
    activeData = organicDepartmentMap;
  } else if (item.section === "Departmental") {
...
// }
    ...
    // 🔥 OFFER FIELDS — MUST be converted from nested structure
    if (data.in_offer && data.offer) {
      formData.append("offer", JSON.stringify(data.offer));
    }
    ...
    const variantsPayload = (data.color_variants || []).map((variant, index) => ({
      ...
      image_field: `variant_image_${index}`, // 👈 link to the file key
    }));
    ...
    (data.color_variants || []).forEach((variant, index) => {
      if (variant.color_image instanceof File) {
        formData.append(`variant_image_${index}`, variant.color_image);
      }
    });
    ...
    // brand an shipping dimensions
    // Shipping dimension (make sure to use the correct field name)
if (data.shipping_dimension_data) {
  formData.append(
    "shipping_dimension_data",
    JSON.stringify({
      length: data.shipping_dimension_data.length,
      width: data.shipping_dimension_data.width,
      height: data.shipping_dimension_data.height,
      weight: data.shipping_dimension_data.weight,
      unit: data.shipping_dimension_data.unit,
      weight_unit: data.shipping_dimension_data.weight_unit,
    })
  );
}
    ...
    if (error.response) {
      showToast("Invalid response. Please coontact us for more information", "error");
    } else {
        ...
        <FormItem className='flex  flex-col'>
          ...
          <FormControl>
            <input
              ...
              value={field.value}
              disabled
              className="rounded p-2 bg-gray-600 text-center outline-none border-none cursor-not-allowed"
            />
          </FormControl>
          <FormMessage />
        </FormItem>
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem className='px-0 m-0'>
          ...
          <FormControl className='flex flex-col justify-end h-65 cursor-not-allowed'>
            <select
            disabled
     ...
     <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea disabled {...field} placeholder="Item description..." />
              </FormControl>
              <FormDescription>
  ...
  <>
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          ...
          <FormDescription>Enter the Shoe type.</FormDescription>
          <FormMessage />
        </FormItem>
      )}
    />
    ...
    <FormField
      ...
      render={({ field }) => {
        ...
        const shoeType = form.watch("shoe_type");
        ...
        const handleCheckboxChange = (checked, size) => {
          if (checked) {
            if (!value.some((v) => v.shoe_size === size)) {
              const newValue = [
                ...value,
                {
                  shoe_type: shoeType || "",
              ...
              console.log("✅ Adding size:", newValue);
              onChange(newValue);
            }
          } else {
            const newValue = value.filter((v) => v.shoe_size !== size);
            console.log("❌ Removing size:", newValue);
            onChange(newValue);
          }
        };
        ...
        return (
          <FormItem>
            ...
            <FormControl>
              <div className="grid grid-cols-3 gap-4 my-2" style={{backgroundColor:colors.primary[500]}}>
                {sizeOptions.map((size) => (
                  <div key={size} className="flex items-center gap-2">
                    <Checkbox
                      id={`size-${size}`}
                      checked={value.some((v) => v.shoe_size === size)}
                      onCheckedChange={(checked) =>
                        handleCheckboxChange(!!checked, size)
                      }
                      style={{ backgroundColor: colors.primary[600] }}
                    />
                    <label className="text-sm mt-2" htmlFor={`size-${size}`}>{size}</label>
                  </div>
                ))}
              </div>
            </FormControl>
            <FormDescription>
              Choose the sizes available.
            </FormDescription>
            <FormMessage />
            {/* 🔍 Debug output */}
            <pre className="text-xs p-2 mt-2 rounded" style={{backgroundColor:colors.primary[500]}}>
        ...
        <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex items-center space-x-2">
                <FormControl>
                  <Checkbox
                    checked={field.value || false}             // Ensure boolean value, no undefined
                    onCheckedChange={(checked) => {            // Normalize event value to boolean
              ...
              <div className="space-y-6">
                ...
                {value.map((variant) => (
                  ...
                  />
                        ...
                        <img
                        src={
                          variant.color_image instanceof File
                            ? URL.createObjectURL(variant.color_image) // ✅ show local file preview
                        ...
                        className="mt-2 w-full h-24 object-cover border rounded"
                      />
                        ...
                        return (
                          <div key={size} className="flex items-center gap-2">
                            <Checkbox
                              ...
                              style={{ backgroundColor: colors.primary[600] }}
                            />
                            <label htmlFor={sizeId} className="text-xs mt-2 ">
                              {size}
                            </label>
                            {selected && (
                              <Input
