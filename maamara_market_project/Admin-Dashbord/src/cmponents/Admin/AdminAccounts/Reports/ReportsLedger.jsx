...
export default function MonthlyReport({ data = {}, loading = false, error = null }) {
  ...
  const cards = [
    {
      ...
      comparison: summary.income?.comparison,
      tone: "bg-green-500/10 text-green-600 dark:text-green-300",
    },
    {
      title: "Expenses",
      icon: arrowDownOutline,
      amount: summary.expenses?.amount,
      comparison: summary.expenses?.comparison,
      tone: "bg-orange-500/10 text-orange-600 dark:text-orange-300",
    },
    {
      title: "Cashbook",
      icon: bookOutline,
      amount: summary.cashbook?.amount,
      comparison: summary.cashbook?.comparison,
      tone: "bg-primary/10 text-primary",
    },
    {
      title: "Vendor Payments",
      icon: walletOutline,
      amount: summary.vendor_payments?.amount,
      comparison: summary.vendor_payments?.comparison,
  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-4 shadow-custom sm:p-5">
      <div className="mb-4">
        <h2 className="text-base font-bold text-card-foreground sm:text-lg">Financial Summary</h2>
        <p className="text-xs text-muted-foreground">
          Income, expenses, cashbook and confirmed vendor settlements.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {cards.map((card) => (
          <article key={card.title} className="min-w-0 rounded-xl border border-border bg-background p-4 text-center">
            ...
            <p className="mt-3 break-words text-lg font-bold tracking-tight text-card-foreground">
              {formatKES(card.amount)}
            </p>
            <p className="mt-2 text-[11px] leading-5 text-muted-foreground">
              {card.comparison || "No data available"}
            </p>
          </article>
        ))}
      </div>
