...
export default function MonthlyReport() {
  ...
  const cards = [
    ...
    },
    {
      ...
      icon: walletOutline,
      amount: paymentsForVendors,
      comparison: "Payments for this month",
    ...
    <section className="rounded-2xl border border-border bg-card p-4 shadow-custom sm:p-5">
      <div className="mb-4">
        ...
        <p className="text-xs text-muted-foreground">
          Income, expenses, cashbook and vendor payments.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {cards.map((card) => (
          <article
            key={card.title}
            className="min-w-0 rounded-xl border border-border bg-background p-4 text-center"
          >
            <div className="flex w-full flex-col items-center justify-center">
              ...
              <div className="mt-2 flex w-full items-center justify-center gap-2">
                ...
                <span
                  className={
                    "rounded-full px-2 py-1 text-[9px] font-bold " + card.tone
                  }
                >
                  KES
                </span>
            ...
            <p className="mt-2 text-[11px] leading-5 text-muted-foreground">
              {card.comparison || "No comparison available"}
            </p>
          </article>
        ))}
      </div>
