    ...
    <>
      {snackbar.open && (
        <div className="fixed right-4 top-20 z-[1400] max-w-sm rounded-[20px] border border-gray-300 bg-card px-4 py-3 text-sm font-semibold text-card-foreground shadow-lg">
          {snackbar.message}
          <button type="button" onClick={() => setSnackbar((prev) => ({ ...prev, open: false }))} className="ml-3 text-xs text-muted-foreground hover:text-card-foreground" aria-label="Dismiss notification">
            ×
          </button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="p-6 text-center">No pending blogs.</div>
      ) : (
            ...
            {blogs.map((blog) => (
              <div key={blog.id} className="border rounded p-4 bg-white shadow">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-500">By: {blog.user_name}</span>
                  <button onClick={() => handleApprove(blog.id)} className="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700">
                    Approve
                  </button>
                </div>
              </div>
